...
def getSinglePost(post_id):
    ...
    if not current_user.is_anonymous:
        if post.user_id == current_user.id:
            return render_template('detail-post.html', post= post, user = current_user, edit=edit, delete = delete)
        else:
...
def loginGoogle(provider):
    ...
    if provider == 'google':
        ...
        try:
            ...
            oauth_flow = flow_from_clientsecrets('./static/google-api.json', scope='')
